...
- (id)initWithDelegate:(id<ZXingDelegate>)scanDelegate showCancel:(BOOL)shouldShowCancel OneDMode:(BOOL)shouldUseoOneDMode withType:(int)aType{
...
- (id)initWithDelegate:(id<ZXingDelegate>)scanDelegate showCancel:(BOOL)shouldShowCancel OneDMode:(BOOL)shouldUseoOneDMode showLicense:(BOOL)shouldShowLicense {
    ...
    if (self) {
        ...
        // 标题所在的view
        UIView *view = [[[UIView alloc]initWithFrame:CGRectMake(0, 20, 320, 48)] autorelease];
        
        view.backgroundColor = [UIColor colorWithRed:51/255.0 green:51/255.0 blue:51/255.0 alpha:1.0];
        
        // 标题
        UILabel *label = [[[UILabel alloc]initWithFrame:CGRectMake(0, 15, 320, 21)] autorelease];
        ...
        [btn setImage:[UIImage imageNamed:@"button_title_back.png"] forState:UIControlStateNormal];
        
        btn.frame = CGRectMake(0, 0, 48, 48);
        
        [btn addTarget:self action:@selector(cancelled) forControlEvents:UIControlEventTouchUpInside];
